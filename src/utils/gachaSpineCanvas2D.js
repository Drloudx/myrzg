 ...
 *
 * ── 为什么存在 ──
 * 用户实测：抽卡演出在 Chrome 正常，**微信内置浏览器（X5 内核）画面渲染不全**
 * （图片以竖条/矩形块出现、有硬直边）。已排除的成因：
 *   - 不是文件没下载完（请求全部 200、content-type 正确）
 *   - 不是画布过大（把微信端 DPR 从 2 降到 1、缓冲 487万→122万像素，无效）
 *   - 不是 WebGL 上下文丢失（未触发 lost）
 * 即：X5 的 WebGL 绘制/纹理路径本身有问题。故在微信内**改用 Canvas2D**——
 * 另一套完全不同的实现路径，微信对 Canvas2D 的支持一向正常。
 *
 * ── 与 WebGL 路径的关系 ──
 ...
 * （实测：角色脸部/身体上成片蛛网状细线，用户明确指出「接缝太明显」）。
 *
 * ── 修法 ──
