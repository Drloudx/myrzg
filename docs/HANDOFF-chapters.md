...
## 三、下一个会话要做的事（用户已提出、尚未做）

用户最近两轮反馈里，**这几项我没动**，因为上下文到上限了：

| # | 问题 | 现状 |
| --- | --- | --- |
| 1 | **副本 / 小地区 / 关卡名的位置没对齐** | 关卡名 `.region-map__node-label`（默认样式）、地区名 `.is-plaque`、副本名 `.is-builtin`，三个都是估的位置 |
| 2 | **副本图标大小不对** | `.is-instance { width: 72px }`，用户觉得和游戏比例不符 |
| 3 | 地区立体图**与附近关卡节点重叠** | 游戏里靠图层遮挡化解，我只调小了尺寸没根治 |
| 4 | 游戏里地区节点还有**「自由探索」小字**和**一圈橙色装饰框** | 完全没做 |

**强烈建议的做法：先量后写。** 这轮宝石位置反复了三遍，就是因为前两次靠估；第三次改成从素材里量包围盒，一次就对。所以：

- 让用户**重新给游戏截图**（要能看清元素相对位置的那种）
- 用 `scripts/dev/scratch/` 下的量测脚本模式（sharp 读图 → 找颜色/透明区域 → 输出百分比），把锚点量出来再写 CSS
