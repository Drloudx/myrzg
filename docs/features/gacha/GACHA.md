...
| `GachaViewport.vue` | 触摸设备竖向窗口整页横置，所有覆盖层共享同一方向 |
...
- 设计坐标以 1534×750、中心为原点，由 `GachaStage` 统一缩放，子组件不重复计算。缩放同时受可用宽、高限制：`contain` 保持固定设计画布，`height` 在宽屏延展背景、窄屏回到完整设计宽度，不再裁掉两侧内容。卡池首页与演出/结算使用 `height` 延展布景，立绘和按钮保持原比例；窗口变化由 ResizeObserver 实时重算。桌面窄窗口仍整体缩小并提示横屏，提示不拦截操作。
- 手机触摸设备在竖向窗口中由 `GachaViewport` 将整页内容旋转 90°，覆盖层与点击区域一起旋转；窗口转为横向后恢复正常方向，不依赖 WebView 的方向锁权限。揭晓小人按自身 CSS 画布尺寸建立 DPR 缓冲区，脚点以台座设计坐标锚定；翻卡背景按有效不透明区域裁切，宽屏两侧不显示透明黑边。翻卡过程中比例变化只更新相机和桌面宽度，不重播动画。
...
- Spine 每帧将 WebGL viewport 同步到当前画布缓冲区；只更新 canvas 尺寸和相机还不够，窗口旋转或共享画布重新挂载后会沿用旧绘图区，造成角色偏移和桌面断口。回归需读取实际 GL viewport，并结合演出截图确认构图。

## 验证与历史参考

改动按范围检查两类卡池、单抽/十连、跳段、完整结果、消耗不足、重置、概率/记录覆盖、声音开关以及窄窗口与横屏。覆盖层既要可见，也要检查其边界落在视口内。反复抽取与退出页面时检查资源释放。

相关入口为 `tests/ui/gacha.spec.js`；窗口适配回归在 `tests/ui/gacha-responsive.spec.js`，检查连续缩窄/放大、两种卡池、概率弹层、十连演出与完整结算的实际边界和操作。`tests/ui/gacha-mobile-layout.spec.js` 覆盖触摸横置、切换方向、超宽背景及固定角色揭晓截图；站台落脚位置需结合截图检查。数据或规则变化另检查对应单元测试，产物变化再执行数据构建。文档整理本身不代表这些运行时验证通过。
