...
- 节点用游戏原图：关卡是石台（从 `MapPanelAtlas` 切出，见下）、地区是立体图（`texture/area/icon/<area.icon>`）、副本入口是立体图（`texture/area/icon/<map.instance[].img>`——**不是** `instance.icon`，后者是副本自己的平面图标）。图下面挂编号/名称牌，名称牌走 `border-image` 用图集里的 `map_a_title`（两端菱形是装饰、中段随名字伸缩，整张缩放会把菱形拉扁）。连线是金色虚线。
- 关卡石台的「三颗星」默认按困难已通关全亮成蓝色：图集里没有蓝色版，所以叠一张同图、`clip-path` 裁到上半再加蓝色滤镜，只染星星不动底座。
...
| `chapters.json`、`stages/{stageId}.json` | 章节与关卡索引、关卡详情（三难度 + 房间/怪物/掉落），按关卡懒加载；索引另含世界地图的底图、拼块矩形、命中归属图，以及每个章节的地区路线图（底图、节点、连线） |
