...
- **蛋池结束状态 = `GetRewardTip`（2026-09-13）**：`GachaPetResult.vue` 还原结算弹层——`mask`(`white` 1534×750 α0.502) + 底板 `item_get` 1534×472（原图 border 全 0，整图横向拉伸是原设计）+ 标题条 `item_get_titel` 220×44 + `ItemBagCell` Large（`item_f_{quality}` 128×128、图标 96×96、连体星条 `com_stars_{rarity}` 68/84/100 @(0,39)、**魔物蛋不显示数量**），逐格 0.1s + `itemGet` 音；点击两段式：第一下补完剩余格（不关）、第二下关闭回到卡池页。角色池仍走 `HeroShowPanel`（`GachaResultPanel.vue`），两者在 `GachaView` 按 `kind` 分流，**不得共用同一面板**。
- **结果一览真值（2026-09-13，第二轮校正）**：结果页是**菱形卡 3-4-3 三行错半格**排布（游戏实际画面；此前本站做成了直立卡片网格，属实现错误）。每张菱形卡 = `gacha_card_botm{rare}` 底板 + 卡面 `gacha_at*.png` + `gacha_card_frame{rare}` 品质边框 + `gacha_card_class{job}`（左上）/ `gacha_card_atr{element}`（右上）角标 + `gacha_card_new`（左侧「新」）/ 碎片图标 + ×N（右下，重复获得）+ **`com_stars_{rare}` 连体星条**（下缘，3/4/5 星 = 96/120/144×48）；**不显示名字文本**。几何：卡 285、列距 340、行距 175、整组居中偏上（避免与右下「招募十次」按钮/消耗行重叠）。入场 `popUpAni` 由 `localScale (0,1,1)` 横向展开，逐格 0.15s 播 `card` 音（**index 1/4/7 跳过音效与间隔**，源码硬编码）。
