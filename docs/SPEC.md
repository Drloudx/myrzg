...
- 技能星阶包含主动技能、天赋、升级计划费用、星阶与碎片消耗；基础属性提供等级模拟、基础/成长/静态属性和累计培养费用。档案显示好感档案及关联剧情，互动按触发场景分组。
- 等级上限取 `playerLevel` 正式可达边界和 `heroRank.heroMaxLevel` 交集。突破按等级门槛自动计入，不提供任意品阶选择；页面称“等级突破”，配置“品阶”只是内部状态。
- 五项成长属性按 `基础值 × (1 + (等级−1)×heroLevel.attUp + 已完成突破attUp累计)` 计算，最终取整遵循 C# 中点取偶数；累计升级与突破费用复用 `calculateStats/calculateUpgradeCosts`。等级、突破、费用不能分别用不一致的上限。
...
入口 `/rewards`，页面 `RewardsView.vue`。读取 `parsed/parsed-pvp.json` 和 `parsed/parsed-hidden.json`，育室槽位消耗取配置。包含挑战赛奖励、隐藏物品点位、育室槽位等分类，未有实际内容的占位不补造奖励。
