...
- **地图视图与列表视图是两屏，互斥**，由显式的视图状态决定（不是从 `chapter` 推导，否则「全部章节」的列表没有可分享的 URL）：地图视图整块区域只有地图，点地块或右上角「展开列表」切到列表；列表视图不出现地图，桌面端搜索框右侧的「进入地图」切回。列表视图无章节时 URL 带 `view=list`，刷新与分享能回到同一屏。
...
- 画布高度按**实测**算出（`左右面板底部 − 画布顶部`，取不到面板时退回 `视口底 − 10`），不用 `100dvh − 常量`：常量估偏几像素画布就会比面板高。画布**底部不留外边距**——留了同样会超出面板，页面多出一条无意义的滚动条。
- 900px 的高度上限是为了限制裁切量：舞台取画布宽高中的较大者，视口越高裁得越多，超过约 900px 会开始啃到大陆边缘而不是云朵。
- 画布描边沿用全局 `.paper-panel`（与左侧导航面板同一套 2px 描边、阴影与圆角），不另写一套面板样式；画布内只有「展开列表」是真按钮，其余覆盖层（标题条、底部标签）都 `pointer-events: none`，不挡地图点击。
