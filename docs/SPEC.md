...
- 章节按钮显示**地区名**（不带宽高代号数字，如「索利德山地」而不是「2 索利德山地」）。
- 关卡卡片只显示**一个**难度标签：按当前难度筛选显示对应难度，筛选为「全部」时默认显示「普通」；关卡没有「普通」时（如序章只有简单）退回它实际有的第一个难度。卡片上的推荐等级跟着这个难度走——三难度的 `battleLevel` 不同，索引里按难度存 `levels`，不用单一等级。
- 关卡详情的难度切换用 `UiFilterRow` + `UiFilterPill`（与列表页的难度筛选同款），不用 `UiSegmentedTabs`——后者是深色木条、为深色底设计，放在羊皮纸弹窗里不搭。详情徽标行沿用各详情页共用的 `.detail-badges`（`display:flex; gap`），漏了这条样式标签之间就只剩空白字符的间距。
...
- **关卡列表的显隐按端区分**：手机端常驻（没有地图，列表就是主视图）；桌面端属于列表视图——由「展开列表」或点地块进入，地图不再出现，搜索框右侧的「进入地图」切回。搜索词在列表视图内生效（地图视图没有搜索框）。

### 章节地图

`/chapters` 的主视图是游戏原版的地图，分两级：世界地图（底图 + 章节拼块）→ 点地区进入该章节的地区路线图。
...
- 地区路线图的节点坐标取 `area.map`，连线直接取 `area.map.link`。**摆放偏移不要照搬源码**：源码给地区节点上移 80、副本入口上移 75，那是给 `AreaItemUI` / `InstanceRoomItemUI` 在它自己的 prefab 层级里用的；网页版是「sprite 居中摆放」，照搬会把图标整体抬离节点。本站两个偏移都取 0，改用各 sprite 自己的锚点定位（见下条）。探索点只画点不标名——`explorePoint.json` 只有一条配置，真正的点位来自玩家存档。
...
- 节点用游戏原图：关卡是石台（图集 `map_s`，见下）、地区是立体图（`texture/area/icon/<area.icon>`）、副本入口是立体图（`texture/area/icon/<map.instance[].img>`——**不是** `instance.icon`，后者是副本自己的平面图标）。图下面挂编号/名称牌，名称牌走 `border-image` 用图集里的 `map_a_title`（两端菱形是装饰、中段随名字伸缩，整张缩放会把菱形拉扁）。连线是金色虚线。
- **三类节点的锚点语义不同，尺寸与锚点都要按各自 sprite 量**（关卡石台 128×128、地区立体图 244×258、副本入口图 172×180）：
  - **关卡**：石台以自身中心落在节点坐标上，名称牌挂在石台下方；编号在游戏里是**没有底板的白字 + 深色描边**，不是深色小牌子。
  - **地区**：锚点在**圆盘中心**（sprite 高度的 75.4% 处，比 sprite 中心低 25.4%），所以 sprite 要往上挪（`top: -17.8px`）才能让圆盘落在节点坐标上；不这么做整棵建筑会往下压、看起来「没落在节点上」。名称牌挂在圆盘下缘，上方还有一块 `map_a_tag` 小标签（游戏里写「自由探索」）。
  - **副本入口**：图以**自身中心**落在节点坐标上，名字压在图上**自带的石牌**里（sprite 的 y 148..165 一带），不再另挂一层边框。
  - 判据来源：`area.map.link` 的端点就是节点坐标本身，把金色虚线打点画出来即可看出端点落在哪。
- 地区名称牌可以**轻微**压到下方关卡石台（游戏本身就这样，石台在上层），但不能整块盖住石台、让水晶从牌子里戳出来——回归断言按「重叠不超过牌子高度的一半」判。
- 图集里的 `map_select_out` / `map_select_in` / `map_select_corner`（两层同心橙环 + 四角回纹角标）是 `AreaItemUI.selectGo` 的**选中态**装饰；本站没有「当前选中地区」这个状态，不显示也不导入。
- **节点锚点只由图形决定**：名称牌绝对定位挂在图形下方；参与布局会把节点整体上推，落点就不准。
- **绝对定位的 `<button>` 里不要用百分比宽度**：按钮是 shrink-to-fit，与子元素的百分比宽度形成循环依赖，浏览器会退化成「按钮撑满、内容居中」。「世界地图」标题条就踩过这个坑，给按钮显式宽度即可。
- 关卡石台不是独立素材，在 `atlas/uiatlas/mappanel/MapPanelAtlas.png` 里。矩形**直接取图集的 sprite 表**（`mSprites` 的 `map_s` / `map_s_lock` / `map_s_1st`，源码用的就是这些 sprite 名），不要另外用连通域或估的坐标——旧常量 `(402,1004,126,126)` 与真实来源不符。裁剪必须用**完整尺寸**那张图集（同目录另有 2047×1389 的缩略版，sprite 表的 y 会越界）。
...
- 地区底图按 `bgPos` 居中摆放，宽度用图片自身像素换算成画布百分比，所以产物里不必存底图宽高。底图路径按**章节 id**（`map_w1_c1_bg`）而不是地区 id（`c1_map`）——两者差一个 `_map` 后缀，拼错时 SPA fallback 会对不存在的图片返回 200 + HTML，只有解码失败才会暴露。
- **地图视图与列表视图是两屏，互斥**，由显式的视图状态决定（不是从 `chapter` 推导，否则「全部章节」的列表没有可分享的 URL）：地图视图整块区域只有地图，点地块进入该地区的路线图、点路线图上的关卡节点打开关卡详情，右上角「展开列表」切到列表；列表视图不出现地图，桌面端搜索框右侧的「进入地图」切回。列表视图无章节时 URL 带 `view=list`，刷新与分享能回到同一屏。
- **地图只在桌面端出现**：手机宽度下拼块里的章节名只有约 9px 高、读不清，一张读不了的地图占掉大半屏反而更差；手机端恒为列表视图，用章节按钮切章节（「进入地图」也不显示）。断点与 `ChapterMapCanvas` 内的 `767px` 判断一致。
- 地图区高度 = **左右面板底部 − 地图区顶部**，由页面测一次给两个地图组件共用（取左右面板而不是视口：两侧 sticky 面板要留底部安全区，比视口底还高一点，按视口算地图会比面板长出一截）。画布**底部不留外边距**——留了同样会超出面板，页面多出一条无意义的滚动条。上限 900px 是为了限制正方形舞台的裁切量。
- 世界地图铺满内容栏：底图是正方形而区域是长方形，所以画布内部放一个正方形**舞台**按 cover 缩放居中，超出画布的部分裁掉——裁掉的是边缘云朵，大陆本身不动。**命中判定必须用舞台矩形换算**（舞台可能比画布大），不能用画布矩形。
- 画布描边沿用全局 `.paper-panel`（与左侧导航面板同一套 2px 描边、阴影与圆角），不另写一套面板样式；世界地图画布内只有「展开列表」是真按钮，其余覆盖层（标题条、底部标签）都 `pointer-events: none`，不挡地图点击。
- 幽夜古堡、黏滑溪谷不在世界地图上，由页面通过画布的 `extra` 插槽在地图视图里补入口——否则地图视图没有任何路径能进这两个章节（当前两者已进黑名单，入口随之为空）。
...
- 「世界地图 · 选择需要前往的章节」标题条在世界地图画布**左上角**（同时是「你在这里」的标识）；在地区路线图里它是**「返回世界地图」按钮**，当前章节标签在底部居中。世界地图上其余覆盖层都 `pointer-events: none`，不挡地图点击。
...
- 鼠标进入地图后，指着的那块**浮起**（放大 1.045、提亮、加投影、提到最上层），其余压暗到 0.55——默认状态所有拼块同色，没有这个反馈就分不清指着哪一块。
...
- 从图集切 sprite 时，**裁剪矩形以图集自己的 sprite 表为准**（`mSprites` 的 `name/x/y/width/height`，源码引用的也是这张表里的 sprite 名），不要另外用连通域或估的坐标。矩形要核对内容包围盒：切到空白区域时产物仍是合法 PNG，只有比对像素才会暴露。同目录常同时存在**缩略版**图集（尺寸更小、y 会越界），裁剪必须用完整尺寸那张。
