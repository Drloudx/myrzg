...
> 相关权威规范：`docs/UI_COMPONENT_LIBRARY.md` §「使用规则·3」、`docs/SPEC.md` 第十节。
...
**代价 / 互斥约束**
"页面滚动"模型（长内容靠页面滚、sticky 标题、剧情/副本地图滚轮）与"锁视口"**互斥**，无法两全。当前按用户要求统一为覆盖式+锁视口，因此：
- 剧情滚轮的"链到页面滚动"用例须重写为"弹窗正文为唯一滚动容器"
- 副本地图改为"普通滚轮不滚页 + `Ctrl`+滚轮缩放"
> 若未来要恢复"最右滚动条控制中间区"，只能回退到页面滚动模型，会重新引入弹窗下方空白/左右栏不固定。
