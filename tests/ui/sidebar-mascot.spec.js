...
test.describe('desktop sidebar mascot', () => {
  ...
  test('batches cover all 34 articulated characters, load visible previews and preserve actions', async ({ page }) => {
    const requests = [], errors = []
    page.on('pageerror', error => errors.push(error.message))
    page.on('request', request => { if (request.resourceType() === 'fetch' && /hero-\d+-idle\.svg/.test(request.url())) requests.push(request.url()) })
    await openPage(page)
    await page.getByRole('button', { name: '暂停希尔待机动画' }).click()
    await selectAction(page, '钓鱼')
    await page.getByRole('button', { name: '切换吉祥物角色' }).click()
    const picker = page.getByRole('dialog', { name: '选择吉祥物' })
    await expect(picker.locator('.mascot-options svg')).toHaveCount(7)
    await expect(page.getByRole('button', { name: '上一批角色' })).toBeDisabled()
    expect(requests.some(url => /hero-005/.test(url))).toBe(false)
    const seen = []
    for (let batch = 0; batch < 5; batch++) {
      await expect(picker.locator('.mascot-batch-position')).toHaveText((batch + 1) + ' / 5')
      await expect(picker.locator('.mascot-options svg')).toHaveCount(batch === 4 ? 6 : 7)
      const options = await picker.locator('.mascot-option').evaluateAll(nodes => nodes.map(el => el.dataset.characterOption))
      seen.push(...options)
      expect(await page.locator('.mascot-art svg').getAttribute('data-character')).toBe('001')
      for (const id of options) {
        const svg = picker.locator('[data-character-option="' + id + '"] svg')
        expect(await svg.evaluate(el => el.querySelectorAll('image, script, foreignObject, filter').length)).toBe(0)
        expect(await svg.evaluate(el => el.getAnimations({ subtree: true }).length)).toBe(0)
      }
      await picker.screenshot({ path: screenshotDir + '/batch-' + (batch + 1) + '.png' })
      if (batch < 4) await page.getByRole('button', { name: '下一批角色' }).click()
    }
    expect(new Set(seen).size).toBe(34)
    await expect(page.getByRole('button', { name: '下一批角色' })).toBeDisabled()
    await page.locator('[data-character-option="066"]').click()
    await expect(picker).toHaveCount(0)
    await expect(page.locator('.mascot-art')).toHaveAttribute('data-action', 'fish')
    await expect(page.locator('.mascot-art svg')).toHaveAttribute('data-character', '066')
    await page.getByRole('button', { name: '选择吉祥物动作' }).click()
    await expect(page.locator('.mascot-action-option')).toHaveCount(4)
    await page.keyboard.press('Escape')
    await selectAction(page, '待机')
    await page.reload()
    await expect(page.locator('.mascot-art')).toHaveAttribute('data-action', 'idle')
    await expect(page.locator('.mascot-art .idle-body')).toHaveCSS('animation-play-state', 'paused')
    await page.getByRole('button', { name: '切换吉祥物角色' }).click()
    await expect(picker.locator('.mascot-batch-position')).toHaveText('5 / 5')
    await page.getByRole('button', { name: '上一批角色' }).click()
    await expect(picker.locator('.mascot-batch-position')).toHaveText('4 / 5')
    await page.keyboard.press('Escape')
    await page.getByRole('button', { name: '播放奥格待机动画' }).click()
    const body = page.locator('.mascot-art .idle-body')
    await expect(body).toHaveCSS('animation-play-state', 'running')
